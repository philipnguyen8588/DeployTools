...
pub async fn open(
    ...
    cols: u32,
    rows: u32,
    // Skip the project auto-cd. Set for terminals opened with a seeded
    // command (docker shell/logs) — the seed does its own `cd`, and a late
    // auto-cd would otherwise land inside a `docker compose exec` container.
    no_auto_cd: bool,
) -> AppResult<TerminalId> {
    // The id is generated by the FRONTEND, which attaches its
    // `term://<id>` listener BEFORE calling this. That ordering is what
    // guarantees the login MOTD (emitted by the driver task the instant
    // the shell produces it) isn't dropped in the window between opening
    // the terminal and the listener being wired up.
    ...
    // Initial auto-cd command for project sessions. Sent as a plain `cd`
    // with NO cursor manipulation: earlier versions appended `clear` (which
    // wiped the whole login MOTD) or a `\033[A\033[2K` cursor-up-erase
    // (which, when the injection landed mid-MOTD — Ubuntu's update-motd can
    // pause >300 ms — erased real MOTD lines). A bare cd can never eat the
    // server's "Welcome to …" / "Last login" text; the one echoed `cd`
    // line is honest and shows the user where the shell landed.
    let init_cmd: Option<String> = if no_auto_cd {
        None
    } else {
        session.project.as_ref().map(|project| {
    ...
    tokio::spawn(async move {
        ...
        // 400 ms following its first output, so it usually lands cleanly at
        // the settled shell prompt (below the fully-painted MOTD) rather
        // than interleaving with it. Since the cd no longer manipulates the
        // cursor, an early injection is at worst cosmetic — never data loss.
